CLASS zcl_fp_tmpl_store_client DEFINITION
  ...
  PRIVATE SECTION.
    DATA:
      ...
      mv_service_id     TYPE string,
      mv_comm_system_id TYPE string.
    METHODS:
      __close_request,
      __conv_path
        IMPORTING
...
CLASS ZCL_FP_TMPL_STORE_CLIENT IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mv_comm_system_id = iv_comm_system_id.
    mv_name = iv_name.
*    TRY.
*        mv_use_dest_srv = iv_use_destination_service.
*        mv_instance_name = iv_service_instance_name.
  ...
  METHOD get_schema_by_name.
    __get_request(
      IMPORTING
        ev_msg     = ev_msg
      RECEIVING
        ro_request = DATA(lo_request) ).

    IF NOT ev_msg IS INITIAL.
      RETURN.
    ENDIF.
  ...
  METHOD __get_request.
    ...
    TRY.
        ...
        ELSE.
          ...
          mo_http_destination = cl_http_destination_provider=>create_by_comm_arrangement(
                                              comm_scenario  = CONV #( mv_instance_name )
                                              service_id     = CONV #( mv_service_id )"'ZADSTEMPLSTORE_REST'
                                              comm_system_id = CONV #( mv_comm_system_id ) )."'TEST_ADOBE' ).

          mv_client = cl_web_http_client_manager=>create_by_http_destination(
                                      i_destination = mo_http_destination ).
